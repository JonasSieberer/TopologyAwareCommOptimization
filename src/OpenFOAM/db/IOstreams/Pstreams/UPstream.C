...
// Read in the custom communication schedule from ./constant
Foam::List<Foam::UPstream::commsStruct> Foam::UPstream::readCustomComm()
{
    fileName scheduleFile("constant/communicationSchedule");

    IFstream is(scheduleFile);
    if (!is.good())
    {
        // If there is no such file use the tree structure as default
        Pout << "This will now use the OpenFOAM standard tree communication schedule!\n\n";

        return calcTreeComm(Pstream::nProcs());
    }

    dictionary dict(is);

    label nDomains(dict.lookup<label>("nDomains"));

    if (nDomains != Pstream::nProcs())
    {
        FatalErrorInFunction
            << "communicationSchedule nDomains = " << nDomains
            << " but running with " << Pstream::nProcs()
            << Foam::abort(FatalError);
    }

    labelList sends(dict.lookup("sends"));
    List<DynamicList<label>> receives(dict.lookup("receives"));

    label nRanks = sends.size();


    // For all processors find the processors it receives data from
    // (and the processors they receive data from etc.)
    List<DynamicList<label>> allReceives(nRanks);
    for (label procID = 0; procID < nRanks; procID++)
    {
        collectReceives(procID, receives, allReceives[procID]);
    }


    List<commsStruct> customCommunication(nRanks);

    for (label procID = 0; procID < nRanks; procID++)
    {
        customCommunication[procID] = commsStruct
        (
            nRanks,
            procID,
            sends[procID],
...
)
{
    ...
    else
    {
        ...
        customCommunication_.append(List<commsStruct>(0));
    }
    ...
    if (UPstream::commSchedule == 3)
    {
        customCommunication_[index] = readCustomComm();
    }
...
)
{
    ...
    customCommunication_[communicator].clear();
...
Foam::DynamicList<Foam::List<Foam::UPstream::commsStruct>>
Foam::UPstream::customCommunication_(10);
...
int Foam::UPstream::commSchedule
(
    Foam::debug::optimisationSwitch("commSchedule", 0)
);
